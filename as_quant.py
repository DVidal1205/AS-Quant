...
def profile_code():
    ...
    ### Change the directory to the input directories, and grab all of the bam files
    current = os.getcwd()
    os.chdir(input1_dir)
    files1 = glob.glob("*.bam")
    os.chdir(os.path.join(current, input2_dir))
    files2 = glob.glob("*.bam")
	os.chdir(current)

	### Generate read coverage files for each chromosome
	def process_file(file, input_dir):
		preprocess.SamtoText(input_dir, current, file, chromosomes)
	
	### Run the process_file function in parallel
	with ThreadPoolExecutor(max_workers=4) as executor:  # Adjust the number of workers as needed
		futures = [executor.submit(process_file, file, input1_dir) for file in files1]
		futures += [executor.submit(process_file, file, input2_dir) for file in files2]
	...
	s1_namelist, s2_namelist = list_dirs(input1_dir), list_dirs(input2_dir)
		...
		def process_samples(sample_list, group, ChromDict_merged, chromosomes, AS, input_dir, species, output_dir):
			for sample in sample_list:
				print(f"Executing: {sample} in {group}")
		...
		if novel.upper() == 'YES':
			...
			target_AS = ['All']
			for AS in target_AS:
				process_samples(s1_namelist, "group 1", ChromDict_merged, chromosomes, AS, input1_dir, species, output_dir)
				process_samples(s2_namelist, "group 2", ChromDict_merged, chromosomes, AS, input2_dir, species, output_dir)
		else:
			print("Running AS-Quant for detecting significant annotated splicing events...")
			for AS in target_AS:
				print(f"Spliced Exon type: {AS}")
				process_samples(s1_namelist, "group 1", ChromDict_merged, chromosomes, AS, input1_dir, species, output_dir)
				process_samples(s2_namelist, "group 2", ChromDict_merged, chromosomes, AS, input2_dir, species, output_dir)
	
		writer_out = pd.ExcelWriter(os.path.join(output_dir, f"asquant_{g1_name}_Vs_{g2_name}.xlsx"), engine='xlsxwriter')
		for AS in target_AS:
			...
			print(f"{AS} counting p-val Complete")
	
			df = pd.read_csv(os.path.join(output_dir, f"{AS}_Output.csv"), delimiter='\t')
			df.sort_values(by=['P_value'], ascending=True, inplace=True)
			df.to_excel(writer_out, sheet_name=AS, index=None, header=True)
			os.remove(os.path.join(output_dir, f"{AS}_Output.csv"))
		writer_out.save()
